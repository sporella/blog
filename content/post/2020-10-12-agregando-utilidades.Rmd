...
3) **Modificar archivo `single.html`:** este archivo determina de qué forma se estructura el formato de la entrada del blog. En mi caso, se encuentra dentro de la carpeta `themes/mi_tema/layouts/post` e indica que pondrá el título, luego la fecha y las etiquetas, luego el contenido. Agregué al final: "Compartir en:" y el código `{{ partial "socialshare.html" . }}`
...
4) **Agregar código html para cuadro de comentarios:** este código lo copié desde la configuración de Utterances y debe ponerse dentro de la carpeta `themes/mi_tema/layouts/partials`. Yo nombré este archivo como `comments.html`
...
3) **Modificar archivo `single.html`:** es el mismo que modificamos en el paso 3 de la sección anterior, el que se encuentra en `themes/mi_tema/layouts/post`. Debe quedar así:
