...
Es muy importante nunca modificar la carpeta del tema directamente, esto por motivos de reproducibilidad, ya que cualquier cambio que haga el creador del tema no podremos trazarlo de buena forma si es que nosotras hicimos algún cambio. 

¿Pero cómo podemos cambiar el tema entonces?

Podemos hacerlo generando una carpeta en el directorio raíz llamada `layouts`. Lo que se encuentre dentro de esta carpeta sobreescribirá las opciones que trae el tema por defecto pero sin modificarlo directamente. Puedes encontrar más información [aquí](https://bookdown.org/yihui/blogdown/custom-layouts.html).
...
_Si no tienes estos íconos puedes seguir los pasos del tutorial de [codingnconcepts.com](https://codingnconcepts.com/hugo/social-icons-hugo/) en donde ingresan los íconos como svg._
...
{{- if .Param "socialshare" }}
  ...
  <section class="social-share">
    <ul class="share-icons">
      ...
      <li>
        <a href="https://twitter.com/intent/tweet?&amp;url={{ $url }}{{.Permalink}}&amp;text={{ .Title }}" target="_blank" rel="noopener" aria-label="Share on Twitter" class="share-btn twitter">
...
2) **Darle estilo a los íconos:** En la misma carpeta, generar un archivo que se llame `style.html` en donde pondremos el código que define el estilo de los íconos.
...
3) **Modificar archivo `single.html`:** este archivo determina de qué forma se estructura el formato de la entrada del blog. En mi caso, lo podremos dentro de `layouts/post` e indica que pondrá el título, luego la fecha y las etiquetas, luego el contenido. Agregué al final: "Compartir en:" y el código `{{ partial "socialshare.html" . }}`

_No confundir el archivo `post/single.html` con el que se encuentra dentro de la carpeta `_default`, ya que este determina la estructura de las páginas que no son entradas del blog._
...
- Revisar que todos los enlaces se generen de forma correcta y funcionen. El código del paso 1 puede variar dependiendo del tema y otros ajustes que se tengan.
...
Utterances funciona utilizando los _issues_ de GitHub, así que los requisitos fundamentales son:

- Tener el repositorio del sitio web alojado en GitHub

- El repositorio debe ser público para que las personas puedan interactuar mediante los _issues_
...
2) **Instalar Utterances** desde [https://utteranc.es/#configuration](https://utteranc.es/#configuration)

4) **Agregar código html para cuadro de comentarios:** este código lo copié desde la configuración de Utterances y debe ponerse dentro de la carpeta `~/layouts/partials`. Yo nombré este archivo como `comments.html`
